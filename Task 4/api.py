...
class API:
  ...
  def __init__(self):
    ...
    self.coord_params_direct = {"q": "", "appid": API_KEY}

    self.coord_params_zip = {"zip": "", "appid": API_KEY}
  ...
  def get_coords(self):
    if self.search_with == "name":
      ...
      response = requests.get(url=ZIP_TO_COORD_URL,
                              params=self.coord_params_zip)
      response.raise_for_status()
  ...
  def get_weather_info(self):
    ...
    print(icon_name)
    icon = requests.get(url="https://openweathermap.org/img/wn/" + icon_name +
                        "@2x.png")
